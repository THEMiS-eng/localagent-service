"""
LocalAgent - CONNECTOR: GitHub (SKILL 7)
...
# Repository configuration - THREE SEPARATE REPOS
REPOS = {
    "service": "THEMiS-eng/localagent-service",
    "dashboard": "THEMiS-eng/localagent-dashboard",
    "chat-module": "THEMiS-eng/ai-chat-module-pro"
}
...
def github_create_repo(repo_name: str, description: str = "", private: bool = False, org: str = None) -> Dict:
    """
    Create a new GitHub repository via API.
    
    Args:
        repo_name: Repository name (e.g., "ai-chat-module-pro")
        description: Repository description
        private: Whether repo is private
        org: Organization name (if creating under org, otherwise user repo)
    ...
    # Try org first if specified
    if org:
        url = f"{GITHUB_API}/orgs/{org}/repos"
        result = _api_request("POST", url, data, token)
        
        # If org creation fails, check why
        if result.get("error"):
            ...
            # Permission denied or not found - try user repo instead
            if "not found" in error_msg or "403" in str(result.get("status", "")):
                # Silent fallback - only report error if user creation also fails
                pass
            else:
                return {"success": False, "error": result.get("error")}
        else:
            # Success with org
            print(f"✅ Created repo '{repo_name}' in org '{org}'")
            return {
                "success": True,
                "exists": False,
    ...
    # Create under user account (fallback or no org specified)
    ...
    if result.get("error"):
        if "already exists" in result.get("error", "").lower():
            # Get username
            user_result = _api_request("GET", f"{GITHUB_API}/user", token=token)
            username = user_result.get("login", "user")
            target = f"{org}/{repo_name}" if org else f"{username}/{repo_name}"
            print(f"✅ Repo '{target}' already exists")
            return {
                "success": True, 
                "exists": True,
        ...
        # Only now report the error
        if org:
            print(f"❌ Failed to create repo '{repo_name}' in org '{org}' and user account")
        return {"success": False, "error": result.get("error")}
    
    print(f"✅ Created repo '{repo_name}' under user account")
    return {
        "success": True,
        "exists": False,
        "url": result.get("html_url", ""),
...
def github_get_file_content(owner: str, repo: str, file_path: str, branch: str = "main") -> Optional[str]:
    """
...
def github_push(source_dir: str, repo_type: str = "service", message: str = None, 
                version: str = None, create_release: bool = False, 
                release_notes: str = None) -> Dict:
    """
    ...
    Args:
        ...
        release_notes: Markdown release notes (auto-generated if not provided)
    ...
    try:
        ...
        # Create release if requested (with zip asset)
        if create_release and version:
            release_result = github_create_release(repo_type, version, notes=release_notes, source_dir=source_dir)
            if release_result.get("success"):
                actions.append(f"release created: {release_result.get('url')}")
                if release_result.get("download_url"):
                    actions.append(f"asset uploaded: {release_result.get('download_url')}")
            else:
...
def github_create_release(repo_type: str, version: str, notes: str = None, 
                          source_dir: str = None) -> Dict:
    """
    Create a GitHub release with optional zip asset.
    
    Args:
        repo_type: "service" or "dashboard"
        version: Version string (e.g., "3.0.31")
        notes: Release notes markdown
        source_dir: Directory to zip and upload as asset
        
    Returns:
        {"success": bool, "url": str, "error": str, "download_url": str}
    """
    if repo_type not in REPOS:
    ...
    # Upload zip asset if source_dir provided
    if source_dir and release_result.get("upload_url"):
        asset_result = _upload_release_asset(
            release_result["upload_url"],
            source_dir,
            f"localagent_{tag}.zip"
        )
        if asset_result.get("success"):
            release_result["download_url"] = asset_result.get("download_url")
            release_result["asset_uploaded"] = True
        else:
            release_result["asset_error"] = asset_result.get("error")
    
    return release_result


def _upload_release_asset(upload_url: str, source_dir: str, filename: str) -> Dict:
    """
    Create zip from source_dir and upload as release asset.
    """
    import tempfile
    import shutil
    import os
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create zip (without .zip extension for shutil)
            base_name = zip_path.replace('.zip', '')
            shutil.make_archive(base_name, 'zip', os.path.dirname(source_dir), 
...
def get_workflow_status(repo: str, commit_sha: str = None) -> str:
    """
    Check GitHub Actions workflow status for a repo.
    ...
    if commit_sha:
        ...
        url += "?per_page=1"  # Just get the latest
    ...
    # Check the latest run (or the one matching commit_sha)
    run = runs[0]
...
def get_workflow_runs(repo: str, limit: int = 10) -> List[Dict]:
    ...
    url = f"{GITHUB_API}/repos/{repo}/actions/runs?per_page={limit}"
    result = _api_request("GET", url)
