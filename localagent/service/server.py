...
# ============================================================
# CACHING SYSTEM - TTL-based cache for frequent data
# ============================================================
...
# Global cache instance (30 second TTL - reduces load on frequent polling)
_cache = TTLCache(ttl_seconds=30)


def cached_get_todo(project: str):
    """Cached todo retrieval."""
    key = f"todo:{project}"
...
def invalidate_cache(category: str = None, project: str = None):
...
async def lifespan(app):
    ...
    # Initialize tracking data if empty (first run)
    todo_file = PROJECTS_DIR / DEFAULT_PROJECT / "TODO.json"
    if not todo_file.exists() or todo_file.stat().st_size < 10:
        try:
            from pathlib import Path
            init_script = Path(__file__).parent.parent.parent / "scripts" / "init_tracking.py"
            if init_script.exists():
                import subprocess
                subprocess.run(["python", str(init_script)], capture_output=True)
                logger.info("Initialized tracking data from history")
        except Exception as e:
            logger.warning(f"Could not init tracking: {e}")
...
from .routers import bugfix as bugfix_router
from .routers import github as github_router
from .routers import debug as debug_router

# Set cache references in routers
todo_router.set_cache(_cache)
bugfix_router.set_cache(_cache)
github_router.set_cache(_cache)
github_router.set_version_helper(get_local_version)
debug_router.set_ws_manager(ws_manager)
debug_router.set_logger(logger)

# Include routers
app.include_router(todo_router.router)
app.include_router(bugfix_router.router)
...
@lru_cache(maxsize=1)
def _get_static_health_info():
    """Cache static health info that doesn't change."""
    return {
        "github_repos": REPOS,
    }
...
async def health():
    ...
    project_dir = PROJECTS_DIR / DEFAULT_PROJECT / "current"
    static_info = _get_static_health_info()
    
    # Cache github version (changes rarely)
    github_version_key = "health:github_version"
    github_version = _cache.get(github_version_key)
    if github_version is None:
        github_version = get_service_version()
        _cache.set(github_version_key, github_version)
    ...
    return {
        ...
        "github_repos": static_info["github_repos"],
...
async def chat_endpoint(data: Dict[str, Any]):
    ...
    Auto-creates TODO/BUGFIX entries for tracking.
    """
    ...
    # === STEP 0: AUTO-DETECT AND CREATE TD/BF ===
    tracking_entry = None
    tracking_type = None
    
    # Detect if this is a fix/bug request
    bug_keywords = ["fix", "bug", "error", "broken", "crash", "issue", "problem", "wrong", "fail", "dupliqué", "duplicate", "missing", "manque", "incorrect"]
    todo_keywords = ["add", "create", "implement", "build", "make", "ajoute", "créer", "nouveau", "new", "feature", "améliore", "improve", "optimize"]
    
    message_lower = message.lower()
    
    if any(kw in message_lower for kw in bug_keywords):
        # Create BUGFIX entry
        from ..engine.tracking import add_bugfix as create_bugfix
        title = message[:80] if len(message) > 80 else message
        bf_id = create_bugfix(DEFAULT_PROJECT, title, message, "chat")
        tracking_entry = {"id": bf_id, "title": title, "type": "bugfix"}
        tracking_type = "BF"
        invalidate_cache("bugfixes", DEFAULT_PROJECT)
        logger.info(f"Auto-created {bf_id}: {title[:50]}")
        
    elif any(kw in message_lower for kw in todo_keywords):
        # Create TODO entry
        title = message[:80] if len(message) > 80 else message
        td_id = add_todo_item(DEFAULT_PROJECT, title, "todo")
        tracking_entry = {"id": td_id, "title": title, "type": "todo"}
        tracking_type = "TD"
        invalidate_cache("todo", DEFAULT_PROJECT)
        logger.info(f"Auto-created {td_id}: {title[:50]}")
    ...
    # Mark tracking entry as done if task succeeded
    if success and tracking_entry:
        from ..engine.tracking import get_todo, save_todo, get_bugfixes, save_bugfixes, apply_bugfix
        ...
        if tracking_type == "TD":
            ...
            invalidate_cache("todo", DEFAULT_PROJECT)
            logger.info(f"Marked {tracking_entry['id']} as done")
            
        elif tracking_type == "BF":
            # Mark bugfix as applied
            ...
            invalidate_cache("bugfixes", DEFAULT_PROJECT)
            logger.info(f"Marked {tracking_entry['id']} as applied")
    ...
    return {
        ...
        "tracking": tracking_entry  # Return tracking info to frontend
    }
...
def get_console_errors_for_claude() -> str:
    """
    Get console errors formatted for Claude context injection.
    Returns recent errors so Claude can see and fix them.
    """
    if not _console_errors:
        return ""
    
    lines = ["## 🚨 Console Errors Detected", ""]
    for err in _console_errors[-5:]:  # Last 5 errors
        ...
        lines.append(f"**[{source.upper()}]** {msg}")
        if file and line:
            lines.append(f"  → {file}:{line}")
        lines.append(f"  → {ts}")
        lines.append("")
    
    lines.append("Please analyze and fix these errors.")
    return "\n".join(lines)
...
async def capture_console_error(data: Dict[str, Any]):
    ...
    Errors are stored for Claude injection AND queued to backlog.
    """
    ...
    # Store for Claude injection (keep last 10)
    _console_errors.append(error_entry)
    if len(_console_errors) > MAX_CONSOLE_ERRORS:
        _console_errors = _console_errors[-MAX_CONSOLE_ERRORS:]
...
async def github_push_endpoint(data: Dict[str, Any]):
    ...
    if result.get("success") and todo_ids:
        from ..engine.tracking import track_pending_release, set_todos_testing
        # Mark TODOs as "testing" immediately
        set_todos_testing(DEFAULT_PROJECT, todo_ids, version)
        # Track for completion when GitHub tests pass
