...
async def apply_bugfix_endpoint(data: Dict[str, Any]):
    ...
    if data.get("push", True):
        ...
        push_result = await loop.run_in_executor(
            None,
            lambda: github_push(service_dir, "service", version=version, create_release=True)
        )
...
async def github_status():
    ...
    return {
        ...
        "dashboard_repo": REPOS["dashboard"],
...
async def init_module_repo(data: Dict[str, Any]):
    ...
    """
    from pathlib import Path
    from ..connectors.github import github_push, github_create_repo, github_repo_exists, REPOS
    ...
    result = await loop.run_in_executor(
        ...
        lambda: github_push(
            str(module_path), 
            repo_key, 
            message=f"Initial release v{version}",
            version=version, 
            create_release=True
        )
    )
    ...
    if result.get("success"):
        logger.info(f"Module {module_name} pushed to GitHub: {repo_full_name}")
