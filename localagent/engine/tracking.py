...
def generate_release_notes(project: str, version: str, since_date: str = None) -> str:
    """
    Generate release notes from completed items AND git changes.
    ...
    # Get last changelog entry to determine since_date if not provided
    if not since_date:
        changelog = get_version_changelog(project)
        if changelog:
            last_release = changelog[0]
            since_date = last_release.get("date", "")
    
    # Get completed TODO items (filter by version or since_date)
    todo = get_todo(project)
    completed_todos = [t for t in todo if t.get("done") and t.get("category") != "nth"]
    
    # Filter by version first, then by date
    version_todos = [t for t in completed_todos if t.get("version") == version]
    if not version_todos and since_date:
        version_todos = [t for t in completed_todos if t.get("completed", "") >= since_date]
    
    # Also include TODOs in "testing" status (being validated)
    testing_todos = [t for t in todo if t.get("status") == "testing"]
    
    all_todos = version_todos + testing_todos
    if all_todos:
        lines.append("### ✅ Completed")
        seen = set()
        for item in all_todos[-10:]:
            title = item.get("title", item.get("task", ""))
            if title and title.strip() and title not in seen:
    ...
    version_nth = [t for t in completed_nth if t.get("version") == version]
    if not version_nth and since_date:
        version_nth = [t for t in completed_nth if t.get("completed", "") >= since_date]
    
    if version_nth:
        lines.append("### 🎁 Nice-to-Have")
        for item in version_nth[-5:]:
            title = item.get("title", item.get("task", ""))
    ...
    # Get applied bugfixes (filter by version or date)
    bugfixes = get_bugfixes(project)
    applied_fixes = [b for b in bugfixes if b.get("status") == "applied"]
    
    version_fixes = [b for b in applied_fixes if b.get("version") == version]
    if not version_fixes and since_date:
        version_fixes = [b for b in applied_fixes if b.get("applied_at", "") >= since_date]
    
    if version_fixes:
        lines.append("### 🐛 Bug Fixes")
        for fix in version_fixes[-5:]:
            desc = fix.get("description", fix.get("title", ""))
    ...
    version_backlog = [b for b in completed_backlog if b.get("version") == version]
    if not version_backlog and since_date:
        version_backlog = [b for b in completed_backlog if b.get("completed", "") >= since_date]
    
    if version_backlog:
        lines.append("### 📋 Backlog")
        for item in version_backlog[-5:]:
    ...
    # If nothing from tracking, try to get git changes
    if not has_content:
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    for commit in commits[:5]:
                        ...
                        if len(parts) > 1:
                            msg = parts[1].strip()
                            if msg and not msg.startswith("Release") and not msg.startswith("v"):
                                lines.append(f"- {msg}")
                    lines.append("")
                    has_content = True
            except:
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if stat_lines:
                        ...
                        if "file" in last_line and "changed" in last_line:
                            lines.append(f"### 📊 Stats")
                            lines.append(f"- {last_line.strip()}")
                            lines.append("")
                            has_content = True
            except:
                pass
    
    # If still nothing, add version-specific generic note
    if not has_content:
