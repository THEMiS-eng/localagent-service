...
def test_direct_modification():
    """Test function to validate negotiation strategies configuration."""
    assert len(NEGOTIATION_STRATEGIES) > 0, "No negotiation strategies defined"
    
    for strategy_name, strategy_config in NEGOTIATION_STRATEGIES.items():
        assert "action" in strategy_config, f"Missing action in {strategy_name}"
        assert "feedback" in strategy_config, f"Missing feedback in {strategy_name}"
        assert "max_retries" in strategy_config, f"Missing max_retries in {strategy_name}"
