...
def install_update() -> Dict:
    ...
    # Find the localagent package directory
    try:
        import localagent
        current_install = Path(localagent.__file__).parent
    except:
        return {"success": False, "error": "Cannot find current installation"}
    ...
    try:
        ...
        print(f"   install_root: {current_install.parent}")
        
        # Get install root (~/localagent_v3)
        install_root = current_install.parent
        ...
        if new_root:
            ...
            print(f"✅ Copied full directory: VERSION, localagent/, dashboard/, modules/")
        else:
        ...
        # Update manifest
        manifest["current_version"] = available_version
        ...
        try:
            ...
            update_version_history(available_version, [
        ...
        print(f"✅ Updated: v{current_version} → v{available_version}")
