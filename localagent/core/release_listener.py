"""
LocalAgent v3.0.34 - CORE: Release Listener
...
def fetch_latest_release(repo_url: str = None) -> Optional[Dict]:
    ...
    Args:
        repo_url: GitHub repo URL or None to use default service repo
    ...
    Returns:
        {
            ...
            "published_at": "2026-01-23T...",
            "download_url": "https://github.com/.../localagent_v3.0.34.zip",
            "assets": [...]
        }
    """
    _ensure_dirs()
    
    # Get repo URL from REPOS config if not provided
    if not repo_url:
        from ..connectors.github import REPOS, _get_token
        repo_url = f"https://github.com/{REPOS['service']}"
    ...
    try:
        ...
        # Include auth token for rate limiting
        from ..connectors.github import _get_token
        token = _get_token()
