"""
LocalAgent v2.10.36 - ROADMAP: Prompt Optimizer (ADVANCED LINTER)
...
FR_INDICATORS = [
    ...
    r"\b(avec|pour|dans|sur|une?|les?|des?|mon|ma|mes|ce|cette)\b",
    r"\b(fichier|page|jeu|projet|application|fonction)\b",
    r"\b(s'il vous plaît|svp|merci)\b",
]

EN_INDICATORS = [
    r"\b(create|generate|write|modify|add|delete|remove|build|make)\b",
    r"\b(with|for|the|a|an|my|this|that)\b",
...
TASK_PATTERNS = {
    "create": {
        "en": [r"\b(create|generate|write|build|make|new)\b"],
        "fr": [r"\b(créer|crée|génère|générer|écris|écrire|construire|nouveau)\b"],
        "output": "file"
    },
    "modify": {
        "en": [r"\b(modify|change|update|edit|fix|refactor|improve)\b"],
        "fr": [r"\b(modifier|changer|mettre à jour|éditer|corriger|améliorer)\b"],
        "output": "diff"
    },
    "explain": {
        "en": [r"\b(explain|describe|what is|how does|tell me about)\b"],
        "fr": [r"\b(explique|expliquer|décris|décrire|c'est quoi|comment)\b"],
        "output": "text"
    },
    "analyze": {
        "en": [r"\b(analyze|review|check|audit|evaluate)\b"],
        "fr": [r"\b(analyser|analyse|vérifier|évaluer|examiner)\b"],
        "output": "report"
    },
    "fix": {
        "en": [r"\b(fix|debug|solve|resolve|repair)\b"],
        "fr": [r"\b(corriger|réparer|résoudre|debugger|fixer)\b"],
        "output": "code"
    }
}
...
def lint_prompt(prompt: str) -> Dict:
    ...
    for rule_name, rule in LINT_RULES.items():
        patterns = rule["patterns"].get(lang, rule["patterns"].get("en", []))
        matches = []
