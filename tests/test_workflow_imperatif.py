...
class TestDashboardProtocolTracking:
    """Test dashboard shows protocol execution in real-time"""
    
    def test_dashboard_has_protocol_tab(self):
        """Dashboard must have Protocol tab"""
        content = Path("dashboard/index.html").read_text()
        assert "Protocol" in content
        assert "tab-protocol" in content
    
    def test_dashboard_has_websocket_connection(self):
        """Dashboard must connect to WebSocket"""
        content = Path("dashboard/index.html").read_text()
        assert "WebSocket" in content
        assert "/ws" in content
        assert "initWebSocket" in content
    
    def test_dashboard_has_protocol_live_status(self):
        """Dashboard must show live execution status"""
        content = Path("dashboard/index.html").read_text()
        assert "protocolLive" in content
        assert "wsStatus" in content
    
    def test_dashboard_has_execution_history(self):
        """Dashboard must show execution history"""
        content = Path("dashboard/index.html").read_text()
        assert "execPanel" in content
        assert "loadExecutionHistory" in content
    
    def test_dashboard_has_violations_panel(self):
        """Dashboard must show violations"""
        content = Path("dashboard/index.html").read_text()
        assert "violationsPanel" in content
        assert "violationsCount" in content
    
    def test_dashboard_handles_protocol_messages(self):
        """Dashboard must handle protocol WebSocket messages"""
        content = Path("dashboard/index.html").read_text()
        assert "handleProtocolMessage" in content
        assert "protocol_start" in content
        assert "protocol_step" in content
        assert "protocol_end" in content
...
class TestServerProtocolAPI:
    ...
    def test_server_has_protocol_history_endpoint(self):
        """Server must have /api/protocol/history endpoint"""
        content = Path("localagent/service/server.py").read_text()
        assert "/api/protocol/history" in content
    
    def test_server_has_protocol_steps_endpoint(self):
        """Server must have /api/protocol/steps endpoint"""
        content = Path("localagent/service/server.py").read_text()
        assert "/api/protocol/steps" in content
    
    def test_server_has_protocol_notify_endpoint(self):
        """Server must have /api/protocol/notify for broadcasting"""
        content = Path("localagent/service/server.py").read_text()
        assert "/api/protocol/notify" in content
    
    def test_server_records_executions(self):
        """Server must record execution history"""
        content = Path("localagent/service/server.py").read_text()
